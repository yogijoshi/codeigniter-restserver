 ...
 * @version		Version 0.2
 * @tutorial    Copied into CodeIgniter-Restserver as per license(allows re-disribution) mentioned above.
 */
...
class Mongo_db {
	 ...
	 */
	 ...
	 public function group($collection = "", $keys = array(),$condition = array()) {

	 	if(empty($collection))
	 		show_error("No Mongo collection selected to insert into", 500);
	 	if(count($keys) == 0 || !is_array($keys))
	 		show_error("Key Not Specified", 500);
	 	
	 	try {
	 		$data=$this->db->{$collection}->group($keys,array ( "count" => 0 ), "function (obj, prev) { prev.count++; }",$condition);
	 		
	 		if(isset($data['retval']))
	 			return($data);
	 		else
	 			return(FALSE);
	 	} catch(MongoCursorException $e) {
	 		show_error("Group of data in MongoDB failed: {$e->getMessage()}", 500);
	 	}
//		db.activity.group(
//           {key: { deal_id:true},
//            cond: { type:'dealview','deal_id':'4d389d3c56d3b441060d0000' },
...
//            });
	...
	private function connection_string() {
		...
		if(isset($this->dbname) && $this->dbname=="dealtadka_staging")
